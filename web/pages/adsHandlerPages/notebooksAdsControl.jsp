...
<%
        ...
        for(FileItem item:items) {
            if (item.isFormField()) {
                ...
                if (!contentType.equals("image/png")) {
                    ...
                    File file = new File("D:\\9. IntelliJ IDEA Projects\\website1\\web\\webImages\\notebook\\" + name + "");
                    while (file.exists()) {
                        name = item.getName().replace(".png", i + ".png");
                        file = new File("D:\\9. IntelliJ IDEA Projects\\website1\\web\\webImages\\notebook\\" + name + "");
                        i++;
                    }
                    item.write(file);
    ...
    Notebook notebook = new Notebook(formData.get(0), Integer.parseInt(formData.get(1)), formData.get(2), formData.get(3),
            nameOfImageForDB, user);

    //Update an advertisement
    if(formData.get(4).equals("Update")){
        Long id = (Long) session.getAttribute("notebookId");
        notebook.setId(id);
        Object object = notebook;
        userAds.updateAds(object);
        session.removeAttribute("notebookId");
        File file = new File("D:\\9. IntelliJ IDEA Projects\\website1\\web\\webImages\\notebook\\"+formData.get(5)+"");
        if(file.exists()){
            file.delete();
        }
%>
